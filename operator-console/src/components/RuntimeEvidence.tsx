      ...
      <div className="evidence-pane browser-pane">
        ...
        <div className="table-frame evidence-scroll">
          <table className="operations-table evidence-table" data-testid="browser-health">
            <thead><tr><th>账号</th><th>页面</th><th>状态</th><th>观察时间</th></tr></thead>
            <tbody>
              {browserHealth.length === 0 && <tr><td colSpan={4} className="empty-cell">暂无浏览器观察器心跳</td></tr>}
              {browserHealth.map((health) => (
                <tr key={`${health.account_id}:${health.page_role}`}>
                  ...
                  <td><span className={`status-label status-${health.status}`} title={`详情：${health.detail}`}><span aria-hidden="true" />{localizeValue(health.status)}</span></td>
