...
2026-07-19 真实检查点：两个专用 Profile 的 Activity 与 Messages 健康为
`4/4` 已就绪；双向可见关注状态和双向人工私信送达均通过，重开会话后仍可见。
自动回关已暂停。现有最终入站消息在身份绑定恢复时被记为初始基线，持久化聚合
仍为零回复计划、零私信动作租约，因此自动回信尚未形成可见发送证据，也没有
重复发送。当前未配置远程模型凭据和私域目的地；后续只继续自动回信、重载幂等、
联系方式阶段与人工接管门禁，不重复已完成的关注或测试消息。

后续调试已在本机忽略配置中接入 OpenAI 兼容模型，AI 计划和唯一发送租约均
成功创建。扩展也已修复自动绑定开关同步和真实 `conv:<id>` 会话匹配。当前这组
受控会话被 TikTok 限制发送：发送气泡显示警告后消失，另一个 Profile 未收到。
未执行计划应记为 `superseded`，已取得租约记为 `uncertain`，并换用可正常收发的
受控账号继续可见发送与重载幂等验收。
