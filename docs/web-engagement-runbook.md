...
- 修复把 Activity 基线升级为版本 `2`：旧基线强制重新建立；有可见关注行时全部只记为历史基线，空面板需连续稳定 30 秒才可建立空基线。自动回关关闭时仍会完成只读基线，但不会领取租约或点击关注。Chrome 回归最新通过 `109/109`，前端 `35/35`、生产构建和 Android 构建通过。
- 当前服务端两个账号映射均为绑定就绪，Activity/Messages 健康为新鲜 `4/4`；两个账号的 AI 回复和自动回关仍保持关闭。必须让两个 Profile 重载包含版本 `2` 基线的扩展，点击“开始监控”，记录当前事件/计划上界并只读观察至少 30 秒，确认没有历史事件增长后，才重新开放动作开关。
