...
已消除最主要的生产耦合：VMOS 配置不再伪装 MYT，核心 wheel 不含 Appium，生产说明只指向自主 HTTPS APK，运行版本可审计，服务器部署只有一个 wheel 来源。操作控制 SQL 已从 `AcquisitionService` 提取到独立 repository，保留原有事务、幂等和失败重放语义。

`acquisition_db.py`、`db.py` 的后续拆分继续遵循“随业务改动提取领域接口”的方式；它们仍是结构债务，但不再作为本轮生产切换的阻断项。真实触达保持暂停，未启动设备、未改变已保存断点，也未修改资格与互动逻辑。
